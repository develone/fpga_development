...
def mul_ts(clk,i_rpi2B,reset,tsenable,t_rpi2B):
	...
	inst_0 = ts( clk, reset, tsenable, d0_o, ts0 , rb0_o )
	inst_1 = ts( clk, reset, tsenable, d1_o, ts1 , rb1_o )
	inst_2 = ts( clk, reset, tsenable, d2_o, ts2 , rb2_o )
	inst_3 = ts( clk, reset, tsenable, d3_o, ts3 , rb3_o )		
	inst_4 = ts( clk, reset, tsenable, d4_o, ts4 , rb4_o )
	inst_5 = ts( clk, reset, tsenable, d5_o, ts5 , rb5_o )
	inst_6 = ts( clk, reset, tsenable, d6_o, ts6 , rb6_o )
	inst_7 = ts( clk, reset, tsenable, d7_o, ts7 , rb7_o )
			
	return myhdl.instances()

def mul_ts_tb(clk,i_rpi2B,reset,tsenable,t_rpi2B):
        dut = mul_ts(clk,i_rpi2B,reset,tsenable,t_rpi2B)
...
'''
tb_fsm = traceSignals(mul_ts_tb,clk,i_rpi2B,reset,tsenable,t_rpi2B)
...
def convert():
    tsenable = Signal(bool(0))	
    reset    = ResetSignal( 0, active=1, async=True )		
    toVerilog(mul_ts,clk,i_rpi2B,reset,tsenable,t_rpi2B)
